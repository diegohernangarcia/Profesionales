	...
	<script Language="JavaScript">
			...
			function validar(formulario) {
				...
				if (formulario.NroDoc.value.length < 1)
					...
					if (formulario.FechaNacimiento.value.length < 1)
					{
						alert("La fecha de nacimiento es obligatoria");
						formulario.FechaNacimiento.focus()
						return (false);
					}
						if (formulario.FechaInscripcion.value.length < 1)
					{
						alert("La fecha de inscripcion es obligatoria");
						formulario.FechaInscripcion.focus()
						return (false);
					}
						if (formulario.FechaExpedicion.value.length < 1)
					{
						alert("La fecha expedicion del titulo es obligatoria");
						formulario.FechaExpedicion.focus()
						return (false);
					}
				}
...
<title>ALTAS DE PROFESIONALES</title>
...
<body bgcolor="#cccccc">
	<form method="POST" name="registro" onSubmit="return validar(this)" action="Altas.php">
		...
		<CENTER>
		<input type="submit" value="ENVIAR" name="ok" id="ok" onsubmit="validar(this)">
		<BR>
		<BR>
